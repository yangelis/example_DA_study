...
particle_list = [(ii,jj) for ii, jj in itertools.product(radial_list, theta_list)]
...
for ii, my_list in enumerate(particle_list):
   pd.DataFrame(my_list, 
                columns=['normalized amplitude in xy-plane',
...
for node in root.root.generation(0):
    node.children=[NodeJob(name=f"{child:03}",
                           ...
                           submit_command = f'bsub -q hpc_acc -e %J.err -o %J.out cd {node.path}/{child:03} && {root.template_path}/000_machine_model/run.sh &',
                           log_file=f"{node.path}/{child:03}/log.json",
                           dictionary={'q1':float(myq1), 
                                       'q2':float(myq2)
                                      })
                   for child, (myq1, myq2) in enumerate(itertools.product(q1, q2))]
...
for node in root.root.generation(1):
    node.children=[NodeJob(name=f"{child:03}",
                           ...
                           submit_command = f'bsub -q hpc_acc -e %J.err -o %J.out cd {node.path}/{child:03} && {root.template_path}/001_prepare_tracking_jobs/run.sh &',
