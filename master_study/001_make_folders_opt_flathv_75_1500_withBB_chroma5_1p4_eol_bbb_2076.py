...
# Beam settings
d_config_beambeam["num_particles_per_bunch"] = (1960 * (1.4e11)**2 / 2064)**0.5
...
for idx_job, (track, bunch_nb) in enumerate(itertools.product(track_array, l_bunch_to_scan)):
    ...
    for beam in ["lhcb1", "lhcb2"]:
        d_config_collider["config_beambeam"]["mask_with_filling_pattern"]["i_bunch_b1"] = int(bunch_nb)
        d_config_collider["config_beambeam"]["mask_with_filling_pattern"]["i_bunch_b2"] = int(bunch_nb)
